...
Task decomposition with TodoWrite → Cannot proceed to implementation without it Reason: To structure tasks and enable progress tracking
Execute rule-advisor → Understand task essence and select appropriate rules Reason: To select appropriate rules and understand task essence
Use Edit/Write/MultiEdit → User approval is mandatory Reason: Practicing the most important principle (Research OK, Implementation STOP)
Execute implementation → Cannot complete with quality check errors Reason: To ensure high-quality code
Count file changes → Auto-stop when threshold exceeded Reason: To get confirmation before impact becomes too large
...
rule-advisor agent execution (invoke using Task tool) → JSON response must exist
TodoWrite task → in_progress status must exist
User approval record → Explicit approval before Edit/Write
Quality check results → Cannot complete with errors > 0
Prohibited Actions
Using any type → Causes type check errors Reason: Destroys type safety and causes runtime errors
Using Edit without TodoWrite → Rule violation Reason: Cannot track progress or ensure quality without task management
Edit/Write/MultiEdit without approval → Move to approval waiting Reason: To prevent violating the most important principle
Declaring completion with quality errors → Does not meet completion criteria Reason: To prevent incomplete code from being mixed in
Metacognitive Execution (Mandatory at Task Start)
Metacognition with Information from rule-advisor
Understand taskEssence (task essence)

Distinguish between surface work and fundamental purpose
Judge "quick fix" vs "proper solution"
Confirm applicableRules (applicable rules)

Judge if selected rules are appropriate
Load necessary sections
Recognize pastFailurePatterns (past failures)

Be careful not to repeat same failures
Be conscious of suggested workarounds
Execute firstAction (initial action)

Start with recommended tools
Proceed systematically
Claude Behavior Control (Failure Prevention)
Auto-stop Triggers (Must Stop)
Detecting 5+ file changes: Stop immediately, report impact to user Reason: Large changes require advance planning and review
Same error occurs 3 times: Root cause analysis mandatory Reason: Need root solution, not symptomatic treatment
Detecting excessive unknown type usage: Reconsider type guard design Reason: High possibility of type safety design issues
After editing 3 files: Force TodoWrite update (cannot use next Edit tool without update) Reason: Need to confirm progress and direction
Handling Error-fixing Impulse
Error discovered → Pause
Re-execute rule-advisor
Root cause analysis (repeat "why" 5 times to identify true cause)
Present action plan
